				...
				for (i = 0; i < m; i++)
				{
					for (j = 0; j < m; j++)
					{
						for (k = 0; k < m; k++)
						{
							...
							{
								if (vds[i][j][k].connectivityFlag == UNPROCESSED) // untouched
								{
	...
	do
	{
		...
		for (m = 0; m<num_wave_front; m++)
		{
				...
				vds[i1][j1][k1].hlevel <= ht  && vds[i1][j1][k1].maxError <= error_t)
			{
				...
				{
					id = list[i1][j1][k1][i];
					if (nodeflag[id] != OUTLIERT)
						nlist[num_nlist++] = id;
				}
				vds[i1][j1][k1].connectivityFlag = PROCESSED; // true data
				...
				if (num_nlist< 6)  // extend one ring
				{
					...
					for (k = -len1; k <= len1; k++)
					{
						...
						if (itmp>0 && vds[i2][j2][k2].hlevel <= ht  && vds[i2][j2][k2].maxError <= error_t)
						{
							...
							vds[i2][j2][k2].connectivityFlag = PROCESSED; // true data
						}
						...
						for (i = -len1; i <= len1; i++)
						{
							for (j = -len1; j <= len1; j++)
							{
								for (k = -len1; k <= len1; k++)
								{
									...
									if (vds[i2][j2][k2].connectivityFlag == UNPROCESSED)
									{
										...
										for (l = 0; l<itmp; l++)
											nlist2[num_nlist2++] = list[i2][j2][k2][l];
									}
								}
							}
						}
							...
							for (j = -len2; j <= len2; j++)
							{
								for (k = -len2; k <= len2; k++)
								{
									...
									if (i == len2 || i == -len2 || j == len2 || j == -len2 || k == len2 || k == -len2)
									{
										...
										if (count[i2][j2][k2]>0 && vds[i2][j2][k2].connectivityFlag == UNPROCESSED)
										{
											nid.i = i2, nid.j = j2, nid.k = k2;
											wave_front2[num_wave_front2++] = nid;
